...
State::State(sf::Shader& flash, sf::Shader& redflash, sf::Font& font)
    : room(room0, flags),
      ...
      restartText(font, "Press ENTER to restart or ESC to quit to menu.", 8),
      victoryText(font, "YOU WON!", 32),
      textbox(font),
...
void State::update(float elapsed, sf::View& camera) {
    if (gameMode != MENU) {
        ...
        for (auto& enemy : room.enemies) {
            ...
            if (enemy->name == "blueSlime") {
                ...
                if (blueSlime->is_starting_to_jump) {
                    soundManager.play_jump = true;
                    blueSlime->is_starting_to_jump = false;
                }
            }
            if (enemy->name == "redSlime") {
                RedSlime* redSlime = static_cast<RedSlime*>(enemy.get());
                if (redSlime->is_starting_to_spit) {
                    soundManager.play_spit_fire = true;
                    redSlime->is_starting_to_spit = false;
                }
                for (auto& fireball : redSlime->fireballs)
        ...
        hud.update();
        if (player.isInvincible) player.invincibilityTime();
                ...
                if (player.hitbox.position.x >= asset.pos.x
                    ...
                    if (room.name == "Risorse/maps-09/room3.json") {
                        if (!flags.chest_3_opened) {
                            ...
                            soundManager.play_chest_open = true;
