...
void Room::load(std::string& new_room) {
    ...
    for (int ty = 0; ty < floor_tile_num.y; ty++) {
        for (int tx = 0; tx < floor_tile_num.x; tx++) {
            ...
            if (tileID == "GRASS" || tileID == "GRASS1" || tileID == "GRASSUP")
                tiles.push_back(Tile(tile_pos, grassTexture, textureRect, tileID));
            else
...
void Room::spikesAnimation() {
    if (start_animations) {
        ...
        for (auto& asset : assets)
            if (asset.name == "spikes") spikesVec.push_back(&asset);
        if (!clock_active) {
